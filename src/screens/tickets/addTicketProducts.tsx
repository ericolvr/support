...
const FormSchema = z.object({
    category: z.number({ required_error: 'Selecione a categoria' }),
...
export function AddTicketProducts() {
    ...
    const getCategories = async () => {
        const response = await ApiTicket.GetCategories({ number })
                    ...
                    <div className='flex items-center gap-2 px-4'>
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                ...
                                <BreadcrumbSeparator className='hidden md:block' />
                                <BreadcrumbItem>
                                    <BreadcrumbPage>Produtos Tícket <b>{number}</b></BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
                                    ...
                                    <div className='flex items-center mt-5' key={item.id}>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className='flex flex-col mt-1'>
                                                        <FormLabel htmlFor={`service-select-${item.id}`}>Categoria</FormLabel>
                                                        <Popover open={openService[item.id] || false} onOpenChange={(isOpen) => setOpenService({ ...openService, [item.id]: isOpen })}>
                                                            <PopoverTrigger asChild>
                                                                ...
                                                                >
                                                                    {field.value
                                                                        ? categoriesList.find((service) => service.id === field.value)?.name
                                                                        : 'Selecione o serviço'}
                                        ...
                                        <div className='w-1/2 mr-8'>
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className='flex flex-col mt-1'>
                                                        <FormLabel htmlFor={`service-select-${item.id}`}>Equipamento</FormLabel>
                                                        <Popover open={openService[item.id] || false} onOpenChange={(isOpen) => setOpenService({ ...openService, [item.id]: isOpen })}>
                                                            <PopoverTrigger asChild>
                                                                <Button
                                                                    variant='outline'
